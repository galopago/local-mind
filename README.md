...
needs. Use `link verify-mcp --json` when an agent or script should read
structured `issues` and `next_actions` instead of parsing terminal text.
...
unexpected `Host` headers, and has no authentication. Do not expose it to the internet without adding auth. HTTP write actions require
...
| `link status [--validate]` | Show local readiness, content/page/memory counts, optional validation summary, and next actions. |
...
| `link benchmark ["query"] [--budget small\|medium\|large] [--project slug]` | Measure local cache, search, smart query, bounded agent payloads, graph initial-load payload, graph timings, search backend, packet size, and readiness recommendations for your current wiki. |
...
| `link verify-mcp [--json]` | Verify `link-mcp` version parity, MCP SDK dependency, wiki path, and print MCP config; JSON output includes structured issues and repair actions. |
